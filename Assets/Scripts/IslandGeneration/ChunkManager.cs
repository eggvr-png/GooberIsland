    ...
    public void Generate(Material mat){
        ...
        for (int i = 0, x = 0; x <= resolution; x++) {
            for (int z = 0; z <= resolution; z++) {
                ...
                float seed = ChunkManager.instance.seed;

                float distance = Vector2.Distance(worldCenter, vertexWorldPos);
                float islandMultipler = Mathf.Sin(Mathf.Clamp((1 + distance) / islandRadius, 0, 1) + 90) * Mathf.PerlinNoise((vertexWorldPos.x + seed) * 0.005f, (vertexWorldPos.y + seed) * 0.005f);
                float y = islandMultipler * 150f;
